...
import { ChevronDown, ChevronUp, Edit, GraduationCap, Plus, Trash2, Users, Upload, FileText, Sparkles, Check, X } from "lucide-react";
...
export default function TeacherTests() {
  ...
  const [aiTestId, setAiTestId] = useState<number | null>(null);
  const [aiSampleText, setAiSampleText] = useState("");
  const [aiCandidateQuestions, setAiCandidateQuestions] = useState<any[]>([]);

  const generateAI = trpc.questions.generateAIQuestions.useMutation({
    onSuccess: (data: any) => {
      setAiCandidateQuestions(data.questions ?? []);
      toast.success("تم توليد الأسئلة بنجاح. راجعها أدناه.");
    },
    onError: (e: any) => toast.error(e.message),
  });

  function openAIGenerator(testId: number) {
    setAiTestId(testId);
    setAiSampleText("");
    setAiCandidateQuestions([]);
    setAiDialog(true);
  }

  function acceptAIQuestion(q: any, idx: number) {
    ...
    setAiCandidateQuestions(prev => prev.filter((_, i) => i !== idx));
  }

  function declineAIQuestion(idx: number) {
    setAiCandidateQuestions(prev => prev.filter((_, i) => i !== idx));
    toast.info("تم استبعاد السؤال");
  }
                ...
                <div className="border-t border-border bg-muted/20 p-4">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <Button size="sm" variant="outline" className="gap-1 h-7 text-xs bg-amber-50 hover:bg-amber-100 text-amber-800 border-amber-300 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-800" onClick={() => openAIGenerator(test.id)}>
                        <Sparkles className="w-3 h-3 text-amber-500" />توليد AI
                      </Button>
      ...
      <Dialog open={aiDialog} onOpenChange={setAiDialog}>
        <DialogContent className="max-w-2xl" dir="rtl">
          ...
          <div className="space-y-4 py-2 max-h-[70vh] overflow-y-auto">
            <div className="space-y-2">
              <Label className="text-xs font-semibold text-muted-foreground">أدخل نموذج لسؤال ترغب في توليد أسئلة مشابهة له بالذكاء الاصطناعي:</Label>
              <Textarea
                value={aiSampleText}
                onChange={e => setAiSampleText(e.target.value)}
                placeholder="مثال: Choose the correct answer: I _____ my homework already. (finish, finished, have finished, am finishing)"
                className="mt-1 ltr font-mono text-sm"
                dir="ltr"
                rows={3}
              />
              <Button
                className="w-full gap-2 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white shadow"
                disabled={generateAI.isPending || !aiSampleText.trim()}
                onClick={() => generateAI.mutate({ sampleQuestion: aiSampleText, count: 3 })}
              >
                <Sparkles className="w-4 h-4" />
                {generateAI.isPending ? "جاري التوليد والتحليل..." : "توليد أسئلة مشابهة"}
              </Button>
            </div>

            {aiCandidateQuestions.length > 0 && (
              <div className="space-y-3 pt-4 border-t border-border/60">
                <p className="text-xs font-bold text-muted-foreground">الأسئلة المقترحة (اختر القبول لإضافتها فوراً للاختبار):</p>
                {aiCandidateQuestions.map((q, idx) => (
                  <div key={idx} className="bg-card rounded-xl p-4 border border-amber-500/30 shadow-sm space-y-3 relative overflow-hidden">
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 to-orange-500" />
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <p className="text-sm font-semibold ltr text-right" dir="ltr">{q.questionText}</p>
                        <div className="grid grid-cols-2 gap-1.5 mt-2.5">
                          {["A","B","C","D"].map(opt => (
                            <span key={opt} className={`text-xs px-2.5 py-1.5 rounded-lg border ${q.correctOption === opt ? "bg-emerald-100 text-emerald-800 border-emerald-300 font-bold" : "bg-muted text-muted-foreground border-border/50"}`}>
                              {opt}. {q[`option${opt}` as keyof typeof q]}
                            </span>
                          ))}
                        </div>
                        {q.explanation && <p className="text-xs text-muted-foreground mt-2.5 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 p-2.5 rounded-lg">{q.explanation}</p>}
                      </div>
                      <div className="flex flex-col gap-2 flex-shrink-0">
                        <Button size="sm" className="h-8 gap-1 w-24 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold" onClick={() => acceptAIQuestion(q, idx)}>
                          <Check className="w-3.5 h-3.5" /> قبول
                        </Button>
                        <Button size="sm" variant="outline" className="h-8 gap-1 w-24 text-destructive border-destructive/40 hover:bg-destructive/10 text-xs font-bold" onClick={() => declineAIQuestion(idx)}>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setAiDialog(false)}>إغلاق</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </TeacherLayout>
