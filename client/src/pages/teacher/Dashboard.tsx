...
import LiveLectureBanner from "@/components/portal/LiveLectureBanner";
...
const t = {
  ar: {
    ...
    activeStudents: "الطلاب المتفاعلون",
    explanatoryLessons: "الدروس المشروحة",
    engagementChartTitle: "مقارنة التفاعل العام",
    engagementMetric: "القيمة",
  },
  en: {
    pageTitle: "Dashboard",
    ...
    explanatoryLessons: "Explanatory Lessons",
    engagementChartTitle: "Overall Engagement Overview",
...
export default function TeacherDashboard() {
  ...
  const statCards = [
    ...
    {
      label: curr.totalTests,
      value: statsLoading ? null : stats?.totalTests?.toString(),
      icon: GraduationCap,
    ...
    {
      label: curr.completionRate,
      value: statsLoading ? null : `${stats?.completionRate ?? 0}%`,
      icon: TrendingUp,
    ...
    {
      label: curr.activeStudents,
      value: statsLoading ? null : stats?.activeStudents?.toString(),
      icon: Users,
    ...
    {
      label: curr.explanatoryLessons,
      value: statsLoading ? null : stats?.explanatoryLessons?.toString(),
      icon: BookOpen,
    ...
    <TeacherLayout title={curr.pageTitle}>
      <div dir={lang === "ar" ? "rtl" : "ltr"}>
            ...
            <CardContent>
              <div className="h-64 w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={[
                      { name: curr.totalStudents, value: stats?.totalStudents ?? 0 },
                      { name: curr.activeStudents, value: stats?.activeStudents ?? 0 },
                      { name: curr.publishedUnits, value: stats?.publishedUnits ?? 0 },
                      { name: curr.totalTests, value: stats?.totalTests ?? 0 },
                      { name: curr.explanatoryLessons, value: stats?.explanatoryLessons ?? 0 },
                    ]}
                  ...
                  >
                    ...
                    <Tooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-background border border-border p-2.5 rounded-lg shadow-md text-xs font-semibold">
                              <p className="text-muted-foreground">{payload[0].payload.name}</p>
                              <p className="text-primary font-bold text-sm mt-1">{payload[0].value}</p>
                            </div>
                          );
                        }
                        return null;
                    ...
                    />
                    <Bar dataKey="value" fill="currentColor" className="fill-primary" radius={[6, 6, 0, 0]}>
                      {
                        [0, 1, 2, 3, 4].map((index) => (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-indigo-50/50 dark:to-indigo-950/30 overflow-hidden relative">
