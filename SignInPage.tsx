...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (!loading && isAuthenticated && user) {
      window.location.href = user.role === "admin" ? "/teacher" : "/student";
    }
  ...
  const t = {
    ar: {
      ...
      demoTeacher: "دخول المعلم (مدير)",
      demoStudent: "دخول الطالب",
    ...
    en: {
      ...
      demoTeacher: "Teacher Demo (Admin)",
    ...
    if (!oauthPortalUrl) {
      setTimeout(() => {
        try {
          ...
          let role = "student";
          if (lowerEmail.includes("teacher") || lowerEmail.includes("admin") || lowerEmail.includes("ahmed")) {
            role = "admin";
  ...
  const handleDemoLogin = (role: "teacher" | "student") => {
    setError("");
    setIsSubmitting(true);
    const demoEmail = role === "teacher" ? "teacher@engportal.com" : "student@engportal.com";
    setEmail(demoEmail);
    ...
    if (!oauthPortalUrl) {
      setTimeout(() => {
        try {
          login(demoEmail, "password", role === "teacher" ? "admin" : "student");
        } catch (err: any) {
          setError(err.message || "Login failed");
          setIsSubmitting(false);
        }
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button
                    ...
                    className="text-xs font-semibold h-9 border-dashed border-primary/40 hover:bg-primary/5 hover:border-primary"
                  >
                    {curr.demoTeacher}
                  </Button>
                  <Button
                    ...
                    className="text-xs font-semibold h-9 border-dashed border-indigo-400/40 hover:bg-indigo-500/5 hover:border-indigo-500"
                  >
                    {curr.demoStudent}
                  </Button>
                </div>
